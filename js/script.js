...
if (sessionStorage.getItem("netflixDone") !== "true") {
  window.location.href = "netflix animasi.html"; // balik ke awal
} else {
  // Hapus tanda biar kalau refresh balik ke awal
  sessionStorage.removeItem("netflixDone");
}
...
const title = document.querySelector(".title-above");

BTN.addEventListener('click', () => {
  title.classList.add("hide"); // sembunyikan teks di atas tombol
  BTN.setAttribute('disabled', true);
  MASTER_TL.restart();
...
const butterflies = document.getElementById('butterfly-background');
